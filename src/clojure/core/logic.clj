...
(deftype Pair [lhs rhs]
  ...
  (key [_] lhs)                               ;;;(getKey [_] lhs)
  (val [_] rhs)                               ;;;(getValue [_] rhs)
...
(defn add-var [cs x c]
  (when-not (lvar? x)
    (throw (ArgumentException. (str "constraint store assoc expected logic var key: " x))))      ;;; Error.
  (let [cm (:cm cs)
        km (:km cs)
        cid (:cid cs)
...
(defn lvar
  ([]
     (let [name (str (. clojure.lang.RT (nextID)))]
       (LVar. name nil (.GetHashCode name) nil)))                              ;;; hashCode
  ([name]
     (lvar name true))
  ([name gensym]
     (let [oname name
           name (if gensym
                  ...
                  (str name))]
       (LVar. name oname (.GetHashCode name) nil))))                          ;;; hashCode
...
(defmacro def-arity-exc-helper []
  (if                                                                        ;;; (try
    (clojure.lang.RT/classForName "clojure.lang.ArityException")             ;;;   (Class/forName "clojure.lang.ArityException")
...
(defmacro tabled
  ...
  [args & grest]
  (let [uuid (symbol (str "tabled-" (Guid/NewGuid)))]                     ;;; (UUID/randomUUID)
    `(fn ~uuid [~@args]
       (let [argv# ~args]
         (fn [a#]
